...
namespace FFHPWeb
{
    public partial class adminpurchaseentry : System.Web.UI.Page
    {
        ...
        protected void btndatesubmit_OnClick(object sender, EventArgs e)
        {
            if (TbxPurchaseDate.Text != "")
            {
                if (btndatesubmit.Text == "Submit")
                {
                    lbldatetext.Text = TbxPurchaseDate.Text;
                    divdate.Visible = false;
                    divdatetext.Visible = true;
                    trvendor.Visible = true;
                    btndatesubmit.Text = "Edit Purchase Date";
                }
                else if (btndatesubmit.Text == "Edit Purchase Date")
                {
                    lbldatetext.Text = "";
                    divdate.Visible = true;
                    divdatetext.Visible = false;
                    trvendor.Visible = false;
                    btndatesubmit.Text = "Submit";
                    ddlvendor.SelectedIndex = 0;
                    gvvendorproducts1.DataSource = null;
        ...
        protected void ddlvendor_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                lblerror.Text = "Please select purchase date";
            }
        }
        void Bindvendorproducts()
        {
            DateTime dtpurchasedate = DateTime.ParseExact(TbxPurchaseDate.Text, "MM/dd/yyyy", CultureInfo.InvariantCulture);
                DataTable dt = new DataTable();
                try
                {
                    if (ddlvendor.SelectedIndex > 0)
                    {
                        ...
                        {
                            using (SqlCommand cmd = new SqlCommand("sp_purchase_entry_admin", con))
                            {
                                cmd.CommandType = CommandType.StoredProcedure;
                                ...
                                //cmd.Parameters.Add("@purchase_date", SqlDbType.DateTime).Value = DateTime.ParseExact(TbxPurchaseDate.Text, "MM/dd/yyyy", CultureInfo.InvariantCulture);
                                cmd.Parameters.Add("@purchase_date", SqlDbType.DateTime).Value = dtpurchasedate;
