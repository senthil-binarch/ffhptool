        ...
        public void getOrderDetailsbyorderid()
        {
            ...
            try
            {
                ...
                if (txtidlist.Text != "")
                {
                    ...
                    queryString = @"SELECT DISTINCT z.increment_id as entity_id,z.grand_total, a.customer_id,a.customer_email,c.telephone, CONCAT( IFNULL(c.firstname,''), ' ', IFNULL(c.lastname,'') ) AS customername, CONCAT( IFNULL( c.street, '' ) , ', ', IFNULL( c.city, '' ) , ', ', IFNULL( c.region, '' ) , ', ', IFNULL( c.postcode, '' ) , ', India, T:', IFNULL( c.telephone, '' ),', ', IFNULL( c.fax, '' ) ) AS Address, '' as Name,IFNULL( c.postcode, '' )as Zipcode,b.qty_ordered,b.sku,z.created_at
FROM `sales_flat_order_grid` as z inner join `sales_flat_order` AS a
ON z.entity_id=a.entity_id
INNER JOIN `sales_flat_order_item` AS b ON a.entity_id = b.order_id
INNER JOIN `sales_flat_order_address` AS c ON c.parent_id = b.order_id
...
SELECT DISTINCT z.increment_id as entity_id,z.grand_total, a.customer_id,a.customer_email,c.telephone, CONCAT( IFNULL(c.firstname,''), ' ', IFNULL(c.lastname,'') ) AS customername, CONCAT( IFNULL( c.street, '' ) , ', ', IFNULL( c.city, '' ) , ', ', IFNULL( c.region, '' ) , ', ', IFNULL( c.postcode, '' ) , ', India, T:', IFNULL( c.telephone, '' ),', ', IFNULL( c.fax, '' ) ) AS Address, '' As Name,IFNULL( c.postcode, '' )as Zipcode,CAST( 1 AS DECIMAL( 10, 3 ) ) AS qty_ordered,'alacarte' as sku,z.created_at
FROM `sales_flat_order_grid` as z inner join `sales_flat_order` AS a
ON z.entity_id=a.entity_id
INNER JOIN `sales_flat_order_item` AS b ON a.entity_id = b.order_id
INNER JOIN `sales_flat_order_address` AS c ON c.parent_id = b.order_id
...
'simple', 'bundle', 'grouped'
                    ...
                    if (queryString != "")
                    {
                        ...
                        foreach (var value in distinctRows)
                        {
                            ...
                            _TestPackList["customer_id"] = customer_id.ToString();
                            _TestPackList["customer_email"] = customer_email.ToString();
