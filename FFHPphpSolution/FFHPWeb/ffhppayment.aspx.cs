...
namespace FFHPWeb
{
    public partial class ffhppayment : System.Web.UI.Page
    {
        ...
        public void getOrderDetails()
        {
            ...
            try
            {
                ...
                queryString = @"SELECT  DISTINCT z.increment_id as entity_id, a.customer_id, CONCAT( IFNULL(c.firstname,''), ' ', IFNULL(c.lastname,'') ) AS customername, CONCAT( IFNULL( c.street, '' ) , ', ', IFNULL( c.city, '' ) , ', ', IFNULL( c.region, '' ) , ', ', IFNULL( c.postcode, '' ) , ', India, T:', IFNULL( c.telephone, '' ) ) AS Address, IFNULL( c.postcode, '' )as Zipcode
...
SELECT DISTINCT z.increment_id as entity_id,z.grand_total, a.customer_id, CONCAT( IFNULL(c.firstname,''), ' ', IFNULL(c.lastname,'') ) AS customername, CONCAT( IFNULL( c.street, '' ) , ', ', IFNULL( c.city, '' ) , ', ', IFNULL( c.region, '' ) , ', ', IFNULL( c.postcode, '' ) , ', India, T:', IFNULL( c.telephone, '' ) ) AS Address, 'alacarte' As Name,CONCAT(IFNULL( d.area, '' ), ' ',IFNULL( c.postcode, '' ))as Zipcode,CAST( 1 AS DECIMAL( 10, 3 ) ) AS qty_ordered,'alacarte' as sku
FROM `sales_flat_order_grid` as z inner join `sales_flat_order` AS a
ON z.entity_id=a.entity_id
INNER JOIN `sales_flat_order_item` AS b ON a.entity_id = b.order_id
INNER JOIN `sales_flat_order_address` AS c ON c.parent_id = b.order_id
                    ...
                    if (queryString != "")
                    {
                        ...
                        foreach (var value in distinctRows)
                        {
                            ...
                            if (c == 1)
                            {
                                ...
                                _TestPackList["EBSAmount"] = "";
                            }
                            else
                            {
                                _TestPackList["Amount"] = "";
                                _TestPackList["EBSAmount"] = Amount.ToString("#,##0.00");//String.Format("{ 0:C}", Amount.ToString()); //(Amount.ToString());
                                EBSTotalAmount = EBSTotalAmount + Amount;
                            }
                            testdt.Rows.Add(_TestPackList);
                            TestPack = "";
                        ...
                        {
                            //GVOrderDetails2.FooterRow.Cells[6].Text = TotalAmount.ToString("#,##0.00");
                            GVOrderDetails2.FooterRow.Cells[6].Text = EBSTotalAmount.ToString("#,##0.00");
                        }
                        else
                        {
                            //GVOrderDetails2.FooterRow.Cells[6].Text = "";
