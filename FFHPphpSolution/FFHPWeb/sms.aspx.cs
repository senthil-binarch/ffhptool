...
namespace FFHPWeb
{
    public partial class sms : System.Web.UI.Page
    {
        ...
        public void ServiceCODOrderupdate()
        {
            try
            {
                string curordnum = "0";
                string _newcurordnum = "";
                if (HFCODOrders.Value != "")
                {
                    curordnum = HFCODOrders.Value.ToString();
                }
                //for verification of cod

                string[] _conord=txtidlist.Text.ToString().Split(',');
                string[] _codord = curordnum.ToString().Split(',');

                for (int i = 0; i < _conord.Count(); i++)
                {
                    for (int j = 0; j < _codord.Count(); j++)
                    {
                        if (_conord[i].ToString().Trim() == _codord[j].ToString().Trim())
                        {
                            if (_newcurordnum == "")
                            {
                                _newcurordnum = _codord[j].ToString();
                            }
                            else
                            {
                                _newcurordnum = _newcurordnum + "," + _codord[j].ToString();
                            }
                            break;
                        }
                    }
                ...
                p_streamWriter1.BaseStream.Seek(0, SeekOrigin.End);
                //p_streamWriter1.WriteLine(pss1 + "FFHP Orders " + " on " + DateTime.Today.Date.ToString("dd:MM:yyyy") + "_" + DateTime.Now.TimeOfDay.ToString()+DateTime.Now.ToString("tt")+" #" + txtidlist.Text.ToString()); p_streamWriter1.Flush();
                p_streamWriter1.WriteLine(DateTime.Today.Date.ToString("dd:MM:yyyy") + "_" + DateTime.Now.TimeOfDay.ToString() + DateTime.Now.ToString("tt") + " #" + _newcurordnum.ToString()); p_streamWriter1.Flush();
                p_streamWriter1.Close();
                //lblerror.Text = "Submitted Successfully";
            }
            catch (Exception ex)
        ...
        public void ReadLastCODorders()
        {
            try
            {
                ...
                if (File.Exists(path))
                {
                    ...
                    if (Orders.Count() > 0)
                    {
                        HFCODOrders.Value = Orders.Last();
                    }
                    if (HFCODOrders.Value.ToString() == "0")
                    {
                        HFCODOrders.Value = "";
                    }
                }
        ...
        public DataTable Readsentsms()
        {
            ...
            DataTable smsdt = new DataTable("OrderList");
            //queryString = "SELECT * FROM `ffhp_smslog` WHERE order_date between '" + Convert.ToDateTime(TbxFromDate.Text.ToString()).ToString("yyyy-MM-dd") + "' and '" + Convert.ToDateTime(TbxToDate.Text.ToString()).AddDays(1).ToString("yyyy-MM-dd") + "'";
            queryString = "SELECT * FROM `ffhp_smslog` WHERE order_date between '" + dtf.ToString("yyyy-MM-dd") + "' and '" + dtt.AddDays(1).ToString("yyyy-MM-dd") + "'";
            if (queryString != "")
            {
                ...
                adapteradminmail.Fill(smsdt);

            }
            return smsdt;
            //DataTable smsdt = new DataTable("OrderList");
            //smsdt.Clear();
            //smsdt.Columns.Add("ordernumber");
            //smsdt.Columns.Add("smstype_id");
            //smsdt.Columns.Add("message");
            //try
            //{
            //    string path = Server.MapPath("Images/ffhpsmstransaction.txt");
            //    string[] ord = File.ReadAllLines(path);
            //    for (int i = 0; i < ord.Count(); i++)
            //    {
            //        string[] Orders = ord[i].Split('#');
            //        if (Orders.Count() > 0)
            //        {
            //            DataRow _SmsList = smsdt.NewRow();
            //            string[] val = Orders[0].Split(':');
            //            _SmsList["ordernumber"] = val[1].ToString();
            //            val = Orders[1].Split(':');
            //            _SmsList["smstype_id"] = val[1].ToString();
            //            val = Orders[2].Split(':');
            //            _SmsList["message"] = val[1].ToString();
            //            if (val[1].ToString().Contains("Your message is successfully sent"))
            //            {
            //                smsdt.Rows.Add(_SmsList);
            //            }
            //        }
            //    }
        ...
        public void Sentsmsupdate(string ordernumber,string name,string telephone,string smstypeid,string message,DateTime order_date,int msg_count)
        {
            try
            {
                string path = Server.MapPath("Images/ffhpsmstransaction.txt");
                ...
                p_streamWriter1.WriteLine("OrderNo:"+ordernumber+"#smstype:"+smstypeid+"#message:"+message); 
                p_streamWriter1.Flush();
                p_streamWriter1.Close();
                //lblerror.Text = "Submitted Successfully";
                //below code for smslog write on database

                queryString = @"INSERT INTO `ffhp_smslog` 

(order_date,ordernumber,name,telephone,smstype_id,message,msg_count,created_date)
VALUES('" + order_date.ToString("yyyy-MM-dd hh:mm:ss") + "','" + ordernumber + "','" + name + "','" + telephone + "'," + smstypeid + ",'" + message + "'," + msg_count + ",'" + DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss") + "')";
            ...
            {
                lblerror.Text = ex.ToString();//errormsg.ToString();
            }
...
//) AND c.address_type = 'shipping' AND and a.created_at between '" + Convert.ToDateTime(TbxFromDate.Text.ToString()).ToString("yyyy-MM-dd") + "' and '" + Convert.ToDateTime(TbxToDate.Text.ToString()).ToString("yyyy-MM-dd") + "'";
                    ...
                    //{
                        ...
                        //{
                            foreach (GridViewRow row in GVOrderDetails.Rows)
                            {
                                ...
                                CheckBox CBCODord = (CheckBox)row.FindControl("CBCODorder");
                                CheckBox COD = (CheckBox)row.FindControl("COD");
                                if (s3 != "")
                                {
                                    for (int i = 0; i < codtxtid.Count(); i++)
                                    {
                                        if (s3 == codtxtid[i].ToString())
                                        {
                                            CBCODord.Checked = true;
                                            COD.Checked = true;
                                            COD.Enabled = true;
                                            break;
                                        }
                                        ...
                                        {
                                            CBCODord.Checked = false;
                                        }
                                    }
                                }
                                CheckBox CBConfirmed = (CheckBox)row.FindControl("CBConfirmed");
                                CheckBox CBPacking = (CheckBox)row.FindControl("CBPackingdaysms");
                                CheckBox CBCOD = (CheckBox)row.FindControl("COD");
                                //CheckBox CBDeliverysms = (CheckBox)row.FindControl("CBDeliverysms");
                                CheckBox CBTransitsms = (CheckBox)row.FindControl("CBTransitsms");
                                //CheckBox CBCODDeliverysms = (CheckBox)row.FindControl("CBCODDeliverysms");
                                CheckBox CBDeliveredsms = (CheckBox)row.FindControl("CBDeliveredsms");
                                DataTable smsdt1 = null;
                                smsdt1 = dtsms.AsEnumerable().Where(r => Convert.ToString(r["ordernumber"]) == s3).AsDataView().ToTable();
                                if (smsdt1.Rows.Count == 0)
                                {
                                    if (CB.Checked == false)
                                    {
                                        ...
                                        CBTransitsms.Enabled = false;
                                        //CBCODDeliverysms.Enabled = false;
                                        CBDeliveredsms.Enabled = false;
                                    }
                                }
                                foreach (DataRow smsrow in smsdt1.Rows)
                                {
                                    if (s3 == smsrow["ordernumber"].ToString())
                                    {
                                        if (smsrow["smstype_id"].ToString() == "1")
                                        {
                                            CBConfirmed.Checked = true;
                                            CBConfirmed.Enabled = false;
                                        }
                                        else if (smsrow["smstype_id"].ToString() == "2")
                                        {
                                            CBPacking.Checked = true;
                                            CBPacking.Enabled = false;
                                        }
                                        else if (smsrow["smstype_id"].ToString() == "3")
                                        {
                                            CBTransitsms.Checked = true;
                                            CBTransitsms.Enabled = false;
                                        }
                                        else if (smsrow["smstype_id"].ToString() == "4")
                                        {
                                            CBCOD.Checked = true;
                                            CBCOD.Enabled = false;
                                        }
                                        else if (smsrow["smstype_id"].ToString() == "5")
                                        {
                                            CBDeliveredsms.Checked = true;
                                            CBDeliveredsms.Enabled = false;
                                        }
                                    }
                                }
        ...
        public void getsmscount()
        {
            ...
            baseurl = baseurl.Replace("SMSAPI.jsp?", "balance.jsp?");
            string apiurl = baseurl + "username=" + _username + "&password=" + _password;
        ...
        public void getOrderDetailssupport()
        {
            DataTable dtsms = Readsentsms();
            //if (txtidlist.Text != "")
            //{
            ...
            foreach (GridViewRow row in GVOrderDetails.Rows)
            {
                ...
                CheckBox CBDeliveredsms = (CheckBox)row.FindControl("CBDeliveredsms");
                DataTable smsdt1 = null;
                smsdt1 = dtsms.AsEnumerable().Where(r => Convert.ToString(r["ordernumber"]) == s2).AsDataView().ToTable();
                if (smsdt1.Rows.Count == 0)
                {
                    if (CB.Checked == false)
                    {
                    ...
                    //    CBTransitsms.Enabled = true;
                    //    //CBCODDeliverysms.Enabled = true;
                    //    CBDeliveredsms.Enabled = true;
                    //}
                }
                foreach (DataRow smsrow in smsdt1.Rows)
                {
                    if (s2 == smsrow["ordernumber"].ToString())
                    {
                        switch (Convert.ToInt32(smsrow["smstype_id"].ToString()))
                        {
                            case 0:
                            case 1:
                                {
                            ...
                            case 2:
                                {
                                    if (smsrow["smstype_id"].ToString() == "2")
                                    {
                                        CBPacking.Checked = true;
                                        CBPacking.Enabled = false;
                                    }
                            ...
                            case 3:
                                {
                                    if (smsrow["smstype_id"].ToString() == "3")
                                    {
                                        CBTransitsms.Checked = true;
                                        CBTransitsms.Enabled = false;
                                    }
                            ...
                            case 4:
                                {
                                    if (smsrow["smstype_id"].ToString() == "4")
                                    {
                                        CBCOD.Checked = true;
                                        CBCOD.Enabled = false;
                                    }
                            ...
                            case 5:
                                {
                                    if (smsrow["smstype_id"].ToString() == "5")
                                    {
                                        CBDeliveredsms.Checked = true;
                                        CBDeliveredsms.Enabled = false;
                                    }
...
'simple', 'bundle', 'grouped'
                    ...
                    if (queryString != "")
                    {
                        ...
                        foreach (var value in distinctRows)
                        {
                            ...
                            _TestPackList["Name"] = TestPack.ToString();
                            _TestPackList["Zipcode"] = Zipcode.ToString();
                            int c = Regex.Matches(HFcodorderlist.Value.ToString(), Entityid.ToString()).Count;
                            if (c == 1)
                            {
                        ...
                        GVOrderDetails2.FooterRow.Cells[4].Text = "Total";
                        if (TotalAmount > 0)
                        {
                            //GVOrderDetails2.FooterRow.Cells[6].Text = TotalAmount.ToString("#,##0.00");
                            GVOrderDetails2.FooterRow.Cells[5].Text = TotalAmount.ToString("#,##0.00");
                        }
                        else
                        {
                            //GVOrderDetails2.FooterRow.Cells[6].Text = "";
                            GVOrderDetails2.FooterRow.Cells[5].Text = "";
                        }
...
AND order_id in (" + txtidlist.Text + ")  and Name not like '%Want To Buy This Product/%'  and Name not like 'YOYO%'  and product_id=" + s;
            ...
            //    Response.End(); 
                ...
                for (int j = 0; j < GVOrderDetails2.Columns.Count; j++)
                {
                    ...
                    if (j == GVOrderDetails2.Columns.Count-2)
                    {
                        ...
                        cellText = GVOrderDetails2.FooterRow.Cells[4].Text.ToString();
                    }
                ...
                //Create the PDF Document
                Document pdfDoc = new Document(PageSize.A4.Rotate(), 5f, 10f, 10f, 0f);
                PdfWriter pdfwriter = PdfWriter.GetInstance(pdfDoc, Response.OutputStream);
                pdfwriter.PageEvent = new Footer(Server.MapPath("Images/Calibri.ttf"), 80);
                pdfDoc.Open();
                pdfDoc.Add(table);

                //HeaderFooter footer = new HeaderFooter(new Phrase("AAA"), false);
                ...
                //PdfWriter.GetInstance(pdfDoc, Response.OutputStream).PageEvent = new Footer();

                //var cb = PdfWriter.GetInstance(pdfDoc, Response.OutputStream).DirectContent;
                ////PdfTemplate footerTemplate = cb.CreateTemplate(500, (10 * 45 + 10));
                ////footerTemplate.ShowText("ffhp.in");

                //PdfTemplate footerTmpl = cb.CreateTemplate(100, 10);

                //footerTmpl.ShowText("ffhp.in");
        ...
        //    xmlParse.Flush();
        //}
        protected void CBCODorder_OnCheckedChanged(object sender, EventArgs e)
        {
            GridViewRow gvr = (GridViewRow)(((Control)sender).NamingContainer);
            CheckBox CBCOD = (CheckBox)gvr.FindControl("COD");
            bool cbcodstatus = CBCOD.Checked;
            bool cbcodenable = CBCOD.Enabled;
            if (((CheckBox)gvr.FindControl("CBCODorder")).Checked)
            {
                if (CBCOD.Checked == false)
                {
                    CBCOD.Checked = true;
                    CBCOD.Enabled = true;
                }
            ...
            {
                if (((CheckBox)itm.FindControl("CBCODorder")).Checked)
                {
                    if (((CheckBox)itm.FindControl("CBorder")).Checked)
                    {
                        if (HFCODOrders.Value.ToString().Contains(((HiddenField)itm.FindControl("HFCODordernum")).Value.ToString()) == false)
                        {
                            if (orderlist != "")
                            {
                                orderlist = orderlist + "," + ((HiddenField)itm.FindControl("HFCODordernum")).Value.ToString();
                            }
                            else
                            {
                                orderlist = ((HiddenField)itm.FindControl("HFCODordernum")).Value.ToString();
                            }
                        }
                    }
                    else
                    {
                        lblerror.Text = "This is not a comfirmed order";
                    }
                    //CheckBox CBPacking = (CheckBox)itm.FindControl("CBPackingdaysms");
                    //CheckBox CBCOD = (CheckBox)itm.FindControl("COD");
                    //CheckBox CBDeliverysms = (CheckBox)itm.FindControl("CBDeliverysms");
                    //CheckBox CBCODDeliverysms = (CheckBox)itm.FindControl("CBCODDeliverysms");
                    //CheckBox CBDeliveredsms = (CheckBox)itm.FindControl("CBDeliveredsms");
                    //CBCOD.Enabled = true;
                    //CBPacking.Enabled = true;
                    //CBDeliverysms.Enabled = true;
                    //CBCODDeliverysms.Enabled = true;
                    //CBDeliveredsms.Enabled = true;
                }
                else
                {
                    if (HFCODOrders.Value.ToString().Contains(((HiddenField)itm.FindControl("HFCODordernum")).Value.ToString()) == true)
                    {
                        HFCODOrders.Value = HFCODOrders.Value.ToString().Replace(((HiddenField)itm.FindControl("HFCODordernum")).Value.ToString(), "");
                    }
                    HFCODOrders.Value = HFCODOrders.Value.ToString().Replace(",,", ",");
                    if (HFCODOrders.Value.ToString().EndsWith(","))
                    {
                        HFCODOrders.Value = HFCODOrders.Value.ToString().Remove(HFCODOrders.Value.ToString().Length - 1);
                    }
                    if (HFCODOrders.Value.ToString().StartsWith(","))
                    {
                        HFCODOrders.Value = HFCODOrders.Value.ToString().Remove(0, 1);
                    }
                }
            }
            if (orderlist != "")
            {
                if (HFCODOrders.Value != "")
                {
                    HFCODOrders.Value = HFCODOrders.Value + "," + orderlist;
                }
                else
                {
                    HFCODOrders.Value = orderlist;
                }
            }
            //getOrderDetailssupport();
        }
        public void CBorderOnCheckedChanged(GridViewRow gvr)
        {
            CheckBox CBConfirmed = (CheckBox)gvr.FindControl("CBConfirmed");
            CheckBox CBPacking = (CheckBox)gvr.FindControl("CBPackingdaysms");
            CheckBox CBCOD = (CheckBox)gvr.FindControl("COD");
            //CheckBox CBDeliverysms = (CheckBox)row.FindControl("CBDeliverysms");
            CheckBox CBTransitsms = (CheckBox)gvr.FindControl("CBTransitsms");
            //CheckBox CBCODDeliverysms = (CheckBox)row.FindControl("CBCODDeliverysms");
            CheckBox CBDeliveredsms = (CheckBox)gvr.FindControl("CBDeliveredsms");

            if (((CheckBox)gvr.FindControl("CBorder")).Checked)
            {

                if (CBConfirmed.Checked == false)
                {
                    CBConfirmed.Enabled = true;
                }
                else
                ...
                //CBCODDeliverysms.Enabled = true;
                if (CBDeliveredsms.Checked == false)
                {
                    CBDeliveredsms.Enabled = true;
                }
                else
            ...
            {
                if (CBConfirmed.Enabled == true && CBConfirmed.Checked==true)
                {
                    CBConfirmed.Checked = false;
                }
                CBConfirmed.Enabled = false;
                ...
                if (CBPacking.Enabled == true && CBPacking.Checked == true)
                {
                    CBPacking.Checked = false;
                }
                CBPacking.Enabled = false;

                if (CBTransitsms.Enabled == true && CBTransitsms.Checked == true)
                {
                    CBTransitsms.Checked = false;
                }
                CBTransitsms.Enabled = false;
                ...
                //CBDeliveredsms.Enabled = true;
                if (CBDeliveredsms.Enabled == true && CBDeliveredsms.Checked == true)
                {
                    CBDeliveredsms.Checked = false;
                }
                CBDeliveredsms.Enabled = false;
            ...
            foreach (GridViewRow itm in GVOrderDetails.Rows)
            {
                ...
                else
                {
                    if (((CheckBox)itm.FindControl("CBCODorder")).Checked)
                    {
                        ((CheckBox)itm.FindControl("CBCODorder")).Checked = false;
                    }
        ...
        protected void CBorderall_OnCheckedChanged(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow itm in GVOrderDetails.Rows)
            {
                ...
                {
                    //CheckBox CBorder = ((CheckBox)itm.FindControl("CBorder"));
                    //CBorder.CheckedChanged += CBorder_OnCheckedChanged;
                    ((CheckBox)itm.FindControl("CBorder")).Checked = true;
                    CBorderOnCheckedChanged(itm);
            ...
            if (((CheckBox)GVOrderDetails.HeaderRow.FindControl("CBorderall")).Checked==false)
            {
                ((CheckBox)GVOrderDetails.HeaderRow.FindControl("Confirmedall")).Checked = false;
                ((CheckBox)GVOrderDetails.HeaderRow.FindControl("CBPackingdaysmsall")).Checked = false;
                ((CheckBox)GVOrderDetails.HeaderRow.FindControl("CBTransitsmsall")).Checked = false;
                ((CheckBox)GVOrderDetails.HeaderRow.FindControl("CBDeliveredsmsall")).Checked = false;
            }
            getOrderDetailssupport();
        }
        protected void COD_OnCheckedChanged(object sender, EventArgs e)
        {
            
            string codorderlist = "";
            foreach (GridViewRow itm in GVOrderDetails.Rows)
            {
                if (((CheckBox)itm.FindControl("COD")).Checked)
                {

                    if (codorderlist != "")
                    {
                        codorderlist = codorderlist + "," + ((HiddenField)itm.FindControl("HFcodordernum")).Value.ToString();
                    }
                    else
                    {
                        codorderlist = ((HiddenField)itm.FindControl("HFcodordernum")).Value.ToString();
                    }

                }
            }
            HFcodorderlist.Value = codorderlist.ToString();
        }
        protected void Confirmedall_OnCheckedChanged(object sender, EventArgs e)
        {
            foreach (GridViewRow itm in GVOrderDetails.Rows)
            {
                ...
                {
                    if (((CheckBox)GVOrderDetails.HeaderRow.FindControl("Confirmedall")).Checked)
                    {
        ...
        protected void CBPackingdaysmsall_OnCheckedChanged(object sender, EventArgs e)
        {
            foreach (GridViewRow itm in GVOrderDetails.Rows)
            {
                if (((CheckBox)itm.FindControl("CBPackingdaysms")).Enabled == true)
                {
                    if (((CheckBox)GVOrderDetails.HeaderRow.FindControl("CBPackingdaysmsall")).Checked)
                    {

                        ((CheckBox)itm.FindControl("CBPackingdaysms")).Checked = true;
                    }
                    else
                    {
                        ((CheckBox)itm.FindControl("CBPackingdaysms")).Checked = false;
                    }
                }
            }
        }
        protected void CBTransitsmsall_OnCheckedChanged(object sender, EventArgs e)
        {
            foreach (GridViewRow itm in GVOrderDetails.Rows)
            {
                if (((CheckBox)itm.FindControl("CBTransitsms")).Enabled == true)
                {
                    if (((CheckBox)GVOrderDetails.HeaderRow.FindControl("CBTransitsmsall")).Checked)
                    {

                        ((CheckBox)itm.FindControl("CBTransitsms")).Checked = true;
                    }
                    else
                    {
                        ((CheckBox)itm.FindControl("CBTransitsms")).Checked = false;
                    }
                }
            }
        }
        protected void CBDeliveredsmsall_OnCheckedChanged(object sender, EventArgs e)
        {
            foreach (GridViewRow itm in GVOrderDetails.Rows)
            {
                if (((CheckBox)itm.FindControl("CBDeliveredsms")).Enabled == true)
                {
                    if (((CheckBox)GVOrderDetails.HeaderRow.FindControl("CBDeliveredsmsall")).Checked)
                    {

                        ((CheckBox)itm.FindControl("CBDeliveredsms")).Checked = true;
                    }
                    else
                    {
                        ((CheckBox)itm.FindControl("CBDeliveredsms")).Checked = false;
                    }
                }
            }
        }
        protected void CODall_OnCheckedChanged(object sender, EventArgs e)
        {
        ...
        protected void btnsendsmsCOD_OnClick(object sender, EventArgs e)
        {
            try
            {
                string ConfirmedOrder = "";
                foreach (GridViewRow itm in GVOrderDetails.Rows)
                {
                    if (((CheckBox)itm.FindControl("COD")).Checked && ((CheckBox)itm.FindControl("COD")).Enabled)
                    {
                        if (ConfirmedOrder != "")
                        {
                            ConfirmedOrder = ConfirmedOrder + "," + ((HiddenField)itm.FindControl("HFcodordernum")).Value.ToString();
                        }
                        else
                        {
                            ConfirmedOrder = ((HiddenField)itm.FindControl("HFcodordernum")).Value.ToString();
                        }
                    }
                }
                if (ConfirmedOrder != "")
        ...
        protected void btnsendsmsPackingdaysms_OnClick(object sender, EventArgs e)
        {
            try
            {
                string ConfirmedOrder = "";
                foreach (GridViewRow itm in GVOrderDetails.Rows)
                {
                    if (((CheckBox)itm.FindControl("CBPackingdaysms")).Checked && ((CheckBox)itm.FindControl("CBPackingdaysms")).Enabled)
                    {
                        if (ConfirmedOrder != "")
                        {
                            ConfirmedOrder = ConfirmedOrder + "," + ((HiddenField)itm.FindControl("HFpackingdayordernum")).Value.ToString();
                        }
                        else
                        {
                            ConfirmedOrder = ((HiddenField)itm.FindControl("HFpackingdayordernum")).Value.ToString();
                        }
                    }
                }
                if (ConfirmedOrder != "")
        ...
        protected void btnsendsmstransit_OnClick(object sender, EventArgs e)
        {
            try
            {
                string ConfirmedOrder = "";
                foreach (GridViewRow itm in GVOrderDetails.Rows)
                {
                    if (((CheckBox)itm.FindControl("CBTransitsms")).Checked && ((CheckBox)itm.FindControl("CBTransitsms")).Enabled)
                    {
                        if (ConfirmedOrder != "")
                        {
                            ConfirmedOrder = ConfirmedOrder + "," + ((HiddenField)itm.FindControl("HFtransitordernum")).Value.ToString();
                        }
                        else
                        {
                            ConfirmedOrder = ((HiddenField)itm.FindControl("HFtransitordernum")).Value.ToString();
                        }
                    }
                }
                if (ConfirmedOrder != "")
        ...
        protected void btnsendsmsDelivered_OnClick(object sender, EventArgs e)
        {
            try
            {
                string ConfirmedOrder = "";
                foreach (GridViewRow itm in GVOrderDetails.Rows)
                {
                    if (((CheckBox)itm.FindControl("CBDeliveredsms")).Checked && ((CheckBox)itm.FindControl("CBDeliveredsms")).Enabled)
                    {
                        if (ConfirmedOrder != "")
                        {
                            ConfirmedOrder = ConfirmedOrder + "," + ((HiddenField)itm.FindControl("HFDeliveredordernum")).Value.ToString();
                        }
                        else
                        {
                            ConfirmedOrder = ((HiddenField)itm.FindControl("HFDeliveredordernum")).Value.ToString();
                        }
                    }
                }
                if (ConfirmedOrder != "")
        ...
        public void Sendsms_mail(string OrderNumber,string smstype)
        {
            try
            {
                if (OrderNumber != "")
...
IN ( " + OrderNumber + " )";
                    if (queryString != "")
                    {
                        ...
                        {
                            //_smscontentorg = getsmscontent(smstype);
                            _smsdt = getsmscontent(smstype);
                            if (_smsdt.Rows.Count > 0)
                            {
                                _smscontentorg = _smsdt.Rows[0]["sms_content"].ToString();
                            }
                        }
                        if (_smscontentorg != "")
                        {
        ...
        public void sendmaillikesms(string email,string subject, string body)
        {
            try
            {

                string mailto = "binarch.developer1@gmail.com";//email;//System.Configuration.ConfigurationSettings.AppSettings["Mail_To"].ToString();
                //string mailcc = System.Configuration.ConfigurationSettings.AppSettings["Mail_Cc"].ToString();
                string mailcredential = System.Configuration.ConfigurationSettings.AppSettings["Sms_Mail_Credential"].ToString();
                string mailpassword = System.Configuration.ConfigurationSettings.AppSettings["Sms_Mail_Password"].ToString();
                ...
                //{
                    ...
                    mail.To.Add(mailto);
                    //mail.CC.Add(mailcc);
                    mail.Subject = subject;//Path.GetFileName(filepath);
                    //mail.Body = lblsmsformat.Text.ToString();
                    mail.Body = body;//Path.GetFileName(filepath);
                    mail.IsBodyHtml = true;

                    //var inlineLogo = new LinkedResource("http://192.168.1.14/ffhp/Images/ffhp_main_logo.png");
                    //var inlineLogo = new LinkedResource("http://binarch.com/wp-content/uploads/2013/06/final_logo.png");
        ...
        public string sendsms(string message,string mobilenumber)
        {
            ...
            try
            {
                ...
                //string baseurl = "http://bulksms.mysmsmantra.com:8080/WebSMS/SMSAPI.jsp?username=demouser&password=763475132&sendername=dm&mobileno=918680939328&message=Hello Binarch Test";//DND//Your message is successfully sent to:919999999999
                //string baseurl = "http://bulksms.mysmsmantra.com:8080/WebSMS/sentreport.jsp?username=demouser&password=763475132&fromdate=01-12-2012&todate=30-12-2012";
                //http://bulksms.mysmsmantra.com:8080/WebSMS/SMSAPI.jsp?username=ffhp&password=2042678718&sendername=FFHPIN&mobileno=919840952870&message=Dear MTI DUTT, Thank you for your order. Your Order (No. 100040931) is being processed.
                ...
                //lblsmscount.Text = apiurl;
                //lblsmscount.Visible = true;
                //string s = GetResponse1(apiurl);
                Stream data = client.OpenRead(apiurl);
                StreamReader reader = new StreamReader(data);
        ...
        public static string GetResponse1(string sURL)
        {
            try
            {
                ...
                request.Timeout = Timeout.Infinite;//milliseconds;
        ...
        public DataTable getsmscontent(string smstype)
        {
            //string[] _smsarray = new string[2];
            //string _smscontent = "";
            DataTable dt = new DataTable();
            try
            {
                queryString = @"SELECT * from `1_ffhp_smsformat` where sms_id=" + smstype;
                if (queryString != "")
                {
                    MySqlConnection con = new MySqlConnection(smsconn);
                    con.Open();
                    DataSet smscontent = new DataSet();
                    MySqlDataAdapter adapteradminmail = new MySqlDataAdapter(queryString, smsconn);
                    adapteradminmail.Fill(smscontent, "smscontent");
                    dt = smscontent.Tables["smscontent"];
                    //foreach (DataRow itm in dt.Rows)
                    //{
                    //    _smscontent = itm["sms_content"].ToString();
                        
                    //    _smsarray[0] = itm["sms_content"].ToString();
                    //    _smsarray[1] = itm["sms_type"].ToString();
                    //}
                }
            }
            catch (Exception ex)
            ...
            return dt;//_smsarray;//_smscontent;
        }
        ...
        protected void GVOrderDetails_OnRowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                e.Row.ToolTip = ((HiddenField)e.Row.FindControl("HFTelephone")).Value.ToString();
            }
        }
        //Here below method for api ffhpservice.asmx
        public DataTable API_Readsentsms(DateTime FromDate,DateTime ToDate)
        {

            DataTable smsdt = new DataTable("OrderList");
            queryString = "SELECT * FROM `ffhp_smslog` WHERE order_date between '" + FromDate.ToString("yyyy-MM-dd") + "' and '" + ToDate.AddDays(1).ToString("yyyy-MM-dd") + "'";
            if (queryString != "")
            {
                //ConnectSsh sshobj = new ConnectSsh();
                //SshClient client = sshobj.SshConnect();
        ...
        public DataTable API_getOrderDetailsbyorderid(string ordernumbers,string codordernumbers)
        {
            DataTable DTCustomerDetails = new DataTable("CustomerDetails");
            DataTable testdt = new DataTable("testPackList");
            testdt.Clear();
            testdt.Columns.Add("entity_id");
            testdt.Columns.Add("customername");
...
IN (
'simple', 'bundle', 'grouped'
) AND c.address_type = 'shipping' AND z.increment_id in (" + ordernumbers.ToString() + ")";
                    //Response.Write(queryString);
                    if (queryString != "")
                    {
                        //ConnectSsh sshobj = new ConnectSsh();
                        ...
                        DataSet dsYOYOopt = new DataSet();
                        DataSet dsREADYopt = new DataSet();
                        dsYOYOopt = API_getYOYOwithOptions(ordernumbers);
                        dsREADYopt = API_getREADYwithOptions(ordernumbers);
                        DataTable dtt1 = resort(orderlist1, "entity_id", "ASC");
                        var distinctRows = (from DataRow dRow in dtt1.Rows
                                            select new { col1 = dRow["entity_id"] }).Distinct();
                        ...
                        foreach (var value in distinctRows)
                        {
                            ...
                            _TestPackList["Name"] = TestPack.ToString();
                            _TestPackList["Zipcode"] = Zipcode.ToString();
                            int c = Regex.Matches(codordernumbers.ToString(), Entityid.ToString()).Count;
                            if (c == 1)
                            {
                                _TestPackList["Amount"] = Amount.ToString("#,##0.00");//String.Format("{ 0:C}", Amount.ToString()); //(Amount.ToString());
                                TotalAmount = TotalAmount + Amount;
                        ...
                        DTCustomerDetails = resort(testdt, "entity_id", "ASC");
                        //sshobj.SshDisconnect(client);
                    }
                }
        ...
        public DataSet API_getYOYOwithOptions(string ordernumbers)
        {
            DataSet yoyowopt = new DataSet();
            try
            {
...
INNER JOIN `catalog_product_bundle_selection` AS b ON a.product_id = b.product_id
INNER JOIN `catalog_product_bundle_option_value` AS c ON b.option_id = c.option_id
WHERE a.Name NOT LIKE '%t Want To Buy This Product/' and c.title LIKE '%(Optional)' and z.increment_id in (" + ordernumbers.ToString() + ")";
                //Response.Write(queryString);
        ...
        public DataSet API_getREADYwithOptions(string ordernumbers)
        {
            DataSet READYwopt = new DataSet();
            try
            {
...
and z.increment_id in (" + ordernumbers.ToString() + ")";
        ...
        public DataTable API_getOrderDetails(DateTime FromDate, DateTime ToDate)
        {
            DataTable testdt = new DataTable("testOrderlist");
            testdt.Clear();
            testdt.Columns.Add("entity_id");
...
IN (
'simple', 'bundle', 'grouped'
) AND c.address_type = 'shipping' and z.created_at between '" + FromDate.ToString("yyyy-MM-dd") + "' and '" + ToDate.AddDays(1).ToString("yyyy-MM-dd") + "'";
        ...
        public string API_Sendsms_mail(string OrderNumber, string smstype)
        {
            string message = "";
            try
            {
...
IN ( " + OrderNumber + " )";
                    if (queryString != "")
                    {
                        ...
                        {
                            //_smscontentorg = getsmscontent(smstype);
                            _smsdt = API_getsmscontent(smstype);
                            if (_smsdt.Rows.Count > 0)
                            {
                                _smscontentorg = _smsdt.Rows[0]["sms_content"].ToString();
                            }
                        ...
                        if (_smscontentorg != "")
                        {
                            foreach (DataRow itm in smslist1.Rows)
                            {
                                ...
                                if (telephone.Length >= 10)
                                {
                                    ...
                                    API_Sentsmsupdate(itm["increment_id"].ToString(), itm["customername"].ToString(), itm["telephone"].ToString(), smstype, output, Convert.ToDateTime(itm["created_at"].ToString()), _smscontent.Length);
                                }
        ...
        public DataTable API_getsmscontent(string smstype)
        {
            //string[] _smsarray = new string[2];
            //string _smscontent = "";
            DataTable dt = new DataTable();
        ...
        public void API_Sentsmsupdate(string ordernumber, string name, string telephone, string smstypeid, string message, DateTime order_date, int msg_count)
        {
            try
            {
                string path = Server.MapPath("Images/ffhpsmstransaction.txt");
...
VALUES('" + order_date.ToString("yyyy-MM-dd hh:mm:ss") + "','" + ordernumber + "','" + name + "','" + telephone + "'," + smstypeid + ",'" + message + "'," + msg_count + ",'" + DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss") + "')";
        ...
        public string API_ServiceOrderupdate(string ConfirmOrderNumbers,string CODOrderNumbers)
        {
            string _message = "";
            try
            {
                string curordnum = "0";
                if (ConfirmOrderNumbers.ToString() != "")
                {
                    curordnum = ConfirmOrderNumbers.ToString();
                }
                string path = Server.MapPath("Images/OrderNumbers.txt");
                string pss1 = string.Format("{0:hh:mm}", DateTime.Now);//System.DateTime.Now.ToShortTimeString();
                FileStream pfs1 = new FileStream(path,
                ...
                p_streamWriter1.Close();
                //lblerror.Text = "Submitted Successfully";
                API_ServiceCODOrderupdate(ConfirmOrderNumbers,CODOrderNumbers);
        ...
        public void API_ServiceCODOrderupdate(string ConfirmOrderNumbers,string CODOrderNumbers)
        {
            try
            {
                string curordnum = "0";
                string _newcurordnum = "";
                if (CODOrderNumbers.ToString() != "")
                {
                    curordnum = CODOrderNumbers.ToString();
                }
                //for verification of cod

                string[] _conord = ConfirmOrderNumbers.ToString().Split(',');
                string[] _codord = curordnum.ToString().Split(',');
