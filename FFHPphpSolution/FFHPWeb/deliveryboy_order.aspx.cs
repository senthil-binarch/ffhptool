...
namespace FFHPWeb
{
    public partial class deliveryboy_order : System.Web.UI.Page
    {
        ...
        {
            GVRouteClear.DataSource = get_deliveryboy_routelistnew();
        ...
        public DataSet get_deliveryboy_routecurrentlist()
        {
            string queryString = "";
            DataSet routelist = new DataSet();
            queryString = @"SELECT route_id,route_name FROM `ffhp_route_orders` where deliveryboy_ordernumber is not null and deliveryboy_ordernumber!=''";
            if (queryString != "")
            {
                //ConnectSsh sshobj = new ConnectSsh();
                //SshClient client = sshobj.SshConnect();
        ...
        public DataSet get_deliveryboy_routelistone(int routeid)
        {
            string queryString = "";
            DataSet ordenumberlist = new DataSet();
            queryString = @"SELECT route_id,route_name,deliveryboy_ordernumber,payment_pending_ordernumber,driver_name,driver_phone,deliveryboy_name,deliveryboy_phone FROM `ffhp_route_orders` where route_id=" + routeid;
            if (queryString != "")
            {
                //ConnectSsh sshobj = new ConnectSsh();
                //SshClient client = sshobj.SshConnect();
        ...
        public DataSet get_deliveryboy_routelistnew()
        {
            string queryString = "";
            DataSet ordenumberlist = new DataSet();
            queryString = @"SELECT route_id,route_name,deliveryboy_ordernumber,payment_pending_ordernumber,driver_name,driver_phone,deliveryboy_name,deliveryboy_phone,latitude,longitude FROM `ffhp_route_orders` where deliveryboy_ordernumber is not null and deliveryboy_ordernumber!=''";
            if (queryString != "")
            {
                //ConnectSsh sshobj = new ConnectSsh();
                //SshClient client = sshobj.SshConnect();
...
//) AND c.address_type = 'shipping' AND and a.created_at between '" + Convert.ToDateTime(TbxFromDate.Text.ToString()).ToString("yyyy-MM-dd") + "' and '" + Convert.ToDateTime(TbxToDate.Text.ToString()).ToString("yyyy-MM-dd") + "'";
            ...
            if (ddlroutelist.SelectedIndex != 0)
            {
                if (ddldriver.SelectedIndex != 0)
                {
                    ...
                    if (ddldeliveryboy.SelectedIndex != 0)
                    {
                        ...
                        queryString = @"update `ffhp_route_orders` set deliveryboy_ordernumber='" + txtidlist.Text.ToString() + "', payment_pending_ordernumber='" + txtpaymentpendingordernumber.Text.ToString() + "', driver_name='" + _driver_name + "', driver_phone='" + _driver_phone + "', deliveryboy_name='" + _deliveryboy_name + "', deliveryboy_phone='" + _deliveryboy_phone + "' where route_id=" + ddlroutelist.SelectedValue.ToString() + "";
                        if (queryString != "")
                        {
                            //ConnectSsh sshobj = new ConnectSsh();
                            //SshClient client = sshobj.SshConnect();
        ...
        protected void ddlroutelist_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            string ordernumber= "";
            string payment_pending_ordernumber = "";
            if (ddlroutelist.SelectedIndex != 0)
            {
                ordernumberlist = get_deliveryboy_routelistone(Convert.ToInt32(ddlroutelist.SelectedValue.ToString()));
            }
            if (ordernumberlist.Tables.Count > 0)
            {
                if (ordernumberlist.Tables[0].Rows.Count > 0)
                {
                    ordernumber = ordernumberlist.Tables[0].Rows[0]["deliveryboy_ordernumber"].ToString();
                    payment_pending_ordernumber = ordernumberlist.Tables[0].Rows[0]["payment_pending_ordernumber"].ToString();
            ...
            if (payment_pending_ordernumber != "")
            {
                txtpaymentpendingordernumber.Text = payment_pending_ordernumber;
            }
            else
            {
                txtpaymentpendingordernumber.Text = "";
            }
