...
namespace FFHPWeb
{
    public partial class ffhpnew : System.Web.UI.Page
    {
        ...
        //    //wFile = new FileStream(@"E:\senthil\ffhp\ffhpputty\example.ppk", FileMode.Open);
        //    //Renci.SshNet.PrivateKeyFile obj = new PrivateKeyFile(wFile);

        //    //No suitable authentication method found to complete authentication (publickey,gssapi-keyex,gssapi-with-mic).
        //    //using (var client = new SshClient("www.farmfreshhandpicked.com", "maheswari", "empty")) // establishing ssh connection to server where MySql is hosted

        //    //var keyFile = new PrivateKeyFile(@"E:\senthil\ffhp\ffhpputty\opensshkeynew.ppk");

        //    string filepath=Server.MapPath("privatekey/opensshkeynew.ppk");
        //    var keyFile = new PrivateKeyFile(filepath);
        //    var keyFiles = new[] { keyFile };
        //    //var portForwarded = new ForwardedPortLocal("127.0.0.1", 3307, "127.0.0.1", 3306);
        //    using (var client = new SshClient("www.farmfreshhandpicked.com", "farmfreshhandpicked", keyFiles)) // establishing ssh connection to server where MySql is hosted
        //    {
        ...
        //            portForwarded.Start();
        //            using (MySqlConnection con = new MySqlConnection("SERVER=127.0.0.1;PORT=3306;UID=ffhpmagento;PASSWORD=D6QpT!KDd0dKHI;DATABASE=farmfreshhandpicked"))
        //            {
        //                using (MySqlCommand com = new MySqlCommand("SELECT * FROM 1_ffhp_pincode", con))
        //                {
        ...
        public void ServiceCODOrderupdate()
        {
            try
            {
                ...
                if (HFcodorderlist.Value != "")
                {
                    curordnum = HFcodorderlist.Value.ToString();
                }
                ...
                string[] _codord = txtcodidlist.Text.ToString().Split(',');
                ...
                p_streamWriter1.Close();
                //lblerror.Text = "Submitted Successfully";
                HFcodorderlist.Value = _newcurordnum.ToString();
            }
            catch (Exception ex)
            {
                lblerror.Text = errormsg.ToString();
        ...
        public void ReadLastCODorders()
        {
            try
            {
                ...
                if (File.Exists(path))
                {
                    ...
                    if (Orders.Count() > 0)
                    {
                        HFcodorderlist.Value = Orders.Last();
                        txtcodidlist.Text = Orders.Last();
                    }
                    if (HFcodorderlist.Value.ToString() == "0")
                    {
                        HFcodorderlist.Value = "";
                        txtcodidlist.Text = "";
                    }
                }
                else
        ...
        public void getOrderDetails()
        {
            ...
            try
            {
                ...
                queryString = @"SELECT  DISTINCT z.increment_id as entity_id, a.customer_id, CONCAT( IFNULL(c.firstname,''), ' ', IFNULL(c.lastname,'') ) AS customername, CONCAT( IFNULL( c.street, '' ) , ', ', IFNULL( c.city, '' ) , ', ', IFNULL( c.region, '' ) , ', ', IFNULL( c.postcode, '' ) , ', India, T:', IFNULL( c.telephone, '' ) ) AS Address, b.Name,IFNULL( c.postcode, '' )as Zipcode,a.shipping_description
...
//) AND c.address_type = 'shipping' AND and a.created_at between '" + Convert.ToDateTime(TbxFromDate.Text.ToString()).ToString("yyyy-MM-dd") + "' and '" + Convert.ToDateTime(TbxToDate.Text.ToString()).ToString("yyyy-MM-dd") + "'";
                ...
                if (queryString != "")
                {
                    ...
                    //    var client = new SshClient("www.farmfreshhandpicked.com", "farmfreshhandpicked", keyFiles); // establishing ssh connection to server where MySql is hosted
                    ...
                    //        portForwarded.Start();
                        ...
                        ////SshClient client = sshobj.SshConnect();
                            ...
                            string[] codtxtid = HFcodorderlist.Value.ToString().Split(',');
                            if (txtidlist.Text != "")
                            {
                                ...
                                if (txtid.Count() > 0)
                                {
                                    foreach (GridViewRow row in GVOrderDetails.Rows)
                                    {
                                        ...
                                        CheckBox CBorder = (CheckBox)row.FindControl("CBorder");
                                        ...
                                        //for COD
                                        CheckBox CBCODord = (CheckBox)row.FindControl("COD");
                                        if (s2 != "")
                                        {
                                            for (int i = 0; i < codtxtid.Count(); i++)
                                            {
                                                if (s2 == codtxtid[i].ToString())
                                                {
        ...
        public void getOrderDetailsbyorderid()
        {
            ...
            testdt.Columns.Add("Date");
            testdt.Columns.Add("shipping_description");
            testdt.Columns.Add("customer_as");
            testdt.Columns.Add("goonj");
            testdt.Columns.Add("paymentmethod");
            testdt.Columns.Add("rewardspoints");
            try
            {
                ...
                dt.Columns.Add("Zipcode");
                dt.Columns.Add("Date");
                dt.Columns.Add("shipping_description");
                dt.Columns.Add("customer_as");
                dt.Columns.Add("goonj");
                dt.Columns.Add("paymentmethod");
                dt.Columns.Add("rewardspoints");
                //queryString = "SELECT entity_id, customer_firstname, customer_lastname FROM `sales_flat_order` where created_at between '" + Convert.ToDateTime(TbxFromDate.Text.ToString()).ToString("yyyy-MM-dd") + "' and '" + Convert.ToDateTime(TbxToDate.Text.ToString()).ToString("yyyy-MM-dd") + "'";
                //a.customer_id = c.customer_id AND 
                if (txtidlist.Text != "")
                {
...
//Left outer join bricks_checkout_sale_order as e on a.entity_id=e.order_id
//left outer join bricks_checkout_sale_order_value as f on e.sale_order_id=f.sale_order_value_id
                    //f.value as 'goonj'

                    queryString = @"SELECT DISTINCT z.increment_id as entity_id,z.grand_total, a.customer_id, CONCAT( IFNULL(c.firstname,''), ' ', IFNULL(c.lastname,'') ) AS customername, CONCAT( IFNULL( c.street, '' ) , ', ', IFNULL( c.city, '' ) , ', ', IFNULL( c.region, '' ) , ', ', IFNULL( c.postcode, '' ) , ', India, T:', IFNULL( c.telephone, '' ),', ', IFNULL( c.fax, '' ) ) AS Address, '' as Name,IFNULL( c.postcode, '' )as Zipcode,b.qty_ordered,b.sku,z.created_at,CONCAT( '(', CAST( shipping_description AS CHAR ) , '-', CAST( CAST( a.base_shipping_amount AS DECIMAL( 10, 2 ) ) AS CHAR ) , ')' ) shipping_description,(Case WHEN (select Count(billing_name)from `sales_flat_order_grid` where billing_name=z.billing_name)=1 THEN 'New' Else ''  END) as Customer_as,(Case WHEN message='Goonj' THEN 'EGDG' Else '' END) as goonj,(Case  f.method 
WHEN 'checkmo' THEN 'Card Swiping'
 Else '' END) as method
FROM `sales_flat_order_grid` as z inner join `sales_flat_order` AS a
ON z.entity_id=a.entity_id
Inner join `sales_flat_order_payment` as f ON a.entity_id=f.parent_id
INNER JOIN `sales_flat_order_item` AS b ON a.entity_id = b.order_id
INNER JOIN `sales_flat_order_address` AS c ON c.parent_id = b.order_id
LEFT OUTER JOIN 1_ffhp_pincode as d ON
c.postcode=d.pincode
LEFT OUTER JOIN gift_message as e on a.gift_message_id=e.gift_message_id
WHERE b.parent_item_id IS NULL
AND product_type
IN (
'simple', 'bundle', 'grouped'
) AND c.address_type = 'shipping' AND b.sku not like 'A-la-c%' AND z.increment_id in (" + txtidlist.Text + ")" + @"

UNION ALL

SELECT DISTINCT z.increment_id as entity_id,z.grand_total, a.customer_id, CONCAT( IFNULL(c.firstname,''), ' ', IFNULL(c.lastname,'') ) AS customername, CONCAT( IFNULL( c.street, '' ) , ', ', IFNULL( c.city, '' ) , ', ', IFNULL( c.region, '' ) , ', ', IFNULL( c.postcode, '' ) , ', India, T:', IFNULL( c.telephone, '' ),', ', IFNULL( c.fax, '' ) ) AS Address, '' As Name,IFNULL( c.postcode, '' )as Zipcode,CAST( 1 AS DECIMAL( 10, 3 ) ) AS qty_ordered,'alacarte' as sku,z.created_at,CONCAT( '(', CAST( shipping_description AS CHAR ) , '-', CAST( CAST( a.base_shipping_amount AS DECIMAL( 10, 2 ) ) AS CHAR ) , ')' ) shipping_description,(Case WHEN (select Count(billing_name)from `sales_flat_order_grid` where billing_name=z.billing_name)=1 THEN 'New' Else ' '  END) as Customer_as,(Case WHEN message='Goonj' THEN 'EGDG' Else '' END) as goonj,(Case  f.method 
WHEN 'checkmo' THEN 'Card Swiping'
 Else '' END) as method
FROM `sales_flat_order_grid` as z inner join `sales_flat_order` AS a
ON z.entity_id=a.entity_id
...
IN (
'simple', 'bundle', 'grouped'
) AND c.address_type = 'shipping' AND b.sku like 'A-la-c%' AND z.increment_id in (" + txtidlist.Text + ")";

                    //Response.Write(queryString);//'alacarte' As Name
                    if (queryString != "")
                    {
                        //string filepath = Server.MapPath("privatekey/opensshkeynew.ppk");
                        //var keyFile = new PrivateKeyFile(filepath);
                        ...
                        ////SshClient client = sshobj.SshConnect();
                           ...
                           DataTable dtrewardspoints=new DataTable();
                           string rewardspoints = "";
                            ...
                            dtrewardspoints = getcustomerid_with_Rewardspoints(dtt1);
                            foreach (var value in distinctRows)
                            {
                                ...
                                foreach (DataRow row in products1.Rows)
                                {
                                    ...
                                    updated_at = IndianStandard(Convert.ToDateTime(row["created_at"].ToString()));
                                    shipping_description = row["shipping_description"].ToString();
                                    customer_as = row["customer_as"].ToString();
                                    paymentmethod = row["method"].ToString();
                                    goonj = row["goonj"].ToString();
                                    var rewardsrows = dtrewardspoints.AsEnumerable().Where(r => Convert.ToString(r["customer_id"]) == row["customer_id"].ToString());
                                    if (rewardsrows.Any())
                                    {
                                        rewardspoints = rewardsrows.FirstOrDefault()["rewardspoints"].ToString();
                                    }
                                    else
                                    {
                                        rewardspoints = "";
                                    }
                                    //rewardspoints = dtrewardspoints.AsEnumerable().Where(r => Convert.ToString(r["customer_id"]) == row["customer_id"].ToString()).FirstOrDefault()["rewardspoints"].ToString();
                                    //updated_at = Convert.ToDateTime(row["created_at"].ToString());

                                    //CultureInfo ci = new CultureInfo("hi-IN");
                                    //string date = updated_at.ToString("R", ci);
                                ...
                                if (customer_as.ToString().ToLower() == "new")
                                {
                                    _TestPackList["customername"] = "<b>"+customername.ToString()+"</b>";
                                }
                                else
                                {
                                    _TestPackList["customername"] = customername.ToString();//dtt1.Rows[i]["customername"].ToString();
                                ...
                                _TestPackList["Name"] = TestPack.ToString();
                                _TestPackList["Zipcode"] = Zipcode.ToString();
                                _TestPackList["Date"] = updated_at.ToString();
                                _TestPackList["shipping_description"] = shipping_description.ToString();
                                _TestPackList["customer_as"] = customer_as.ToString();
                                _TestPackList["goonj"] = goonj.ToString();
                                _TestPackList["paymentmethod"] = paymentmethod.ToString();
                                _TestPackList["rewardspoints"] = rewardspoints.ToString();
        ...
        public DataTable getcustomerid_with_Rewardspoints(DataTable dtt1)
        {
            DataTable dtrewardspoints = new DataTable();
            string commaseprated_customer_id = "";

            var distinctcustomerid = (from DataRow dRow in dtt1.Rows
                                      select new { col1 = dRow["customer_id"] }).Distinct();
            
            foreach (var value in distinctcustomerid)
            {
                
                string s = value.col1.ToString();
                if (s != "")
                {
                    if (commaseprated_customer_id == "")
                    {
                        commaseprated_customer_id = s;
                    }
                    else
                    {
                        commaseprated_customer_id = commaseprated_customer_id + "," + s;
                    }
                }
            }

            queryString = "SELECT sum(amount) as rewardspoints,customer_id FROM `m_rewards_transaction` WHERE customer_id in (" + commaseprated_customer_id + ") group by customer_id";
            if (queryString != "")
            {
                MySqlDataAdapter adapteradminmail = new MySqlDataAdapter(queryString, conn);

                adapteradminmail.Fill(dtrewardspoints);

            }
            return dtrewardspoints;
        }
...
AND order_id in (" + txtidlist.Text + ")  and Name not like '%Want To Buy This Product/%'  and Name not like 'YOYO%'  and product_id=" + s;
        ...
        protected void CBorder_OnCheckedChanged(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow itm in GVOrderDetails.Rows)
            {
                ...
                else
                {
                    ...
                    {
                        ((CheckBox)itm.FindControl("COD")).Checked = false;
                    }
        ...
        protected void COD_OnCheckedChanged(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow itm in GVOrderDetails.Rows)
            {
                if (((CheckBox)itm.FindControl("COD")).Checked)
                {
                    if (((CheckBox)itm.FindControl("CBorder")).Checked)
                    {
                        if (HFcodorderlist.Value.ToString().Contains(((HiddenField)itm.FindControl("HFcodordernum")).Value.ToString()) == false)
                        {
                            if (orderlist != "")
                            {
                ...
                {
                    if (HFcodorderlist.Value.ToString().Contains(((HiddenField)itm.FindControl("HFcodordernum")).Value.ToString()) == true)
                    {
                        HFcodorderlist.Value = HFcodorderlist.Value.ToString().Replace(((HiddenField)itm.FindControl("HFcodordernum")).Value.ToString(), "");
                    }
                    HFcodorderlist.Value = HFcodorderlist.Value.ToString().Replace(",,", ",");
                    if (HFcodorderlist.Value.ToString().EndsWith(","))
                    {
                        HFcodorderlist.Value = HFcodorderlist.Value.ToString().Remove(HFcodorderlist.Value.ToString().Length - 1);
                    }
                    if (HFcodorderlist.Value.ToString().StartsWith(","))
                    {
                        HFcodorderlist.Value = HFcodorderlist.Value.ToString().Remove(0, 1);
                    }
                }
            }
            if (orderlist != "")
            {
                if (HFcodorderlist.Value != "")
                {
                    HFcodorderlist.Value = HFcodorderlist.Value + "," + orderlist;
                }
                else
                {
                    HFcodorderlist.Value = orderlist;
                }
            }
            txtcodidlist.Text = HFcodorderlist.Value.ToString();
        ...
        protected void CODall_OnCheckedChanged(object sender, EventArgs e)
        {
            ...
            //((CheckBox)GVOrderDetails.HeaderRow.FindControl("CODall")).Checked

            foreach (GridViewRow itm in GVOrderDetails.Rows)
            {
                if (((CheckBox)GVOrderDetails.HeaderRow.FindControl("CODall")).Checked)
                {
                    ((CheckBox)itm.FindControl("COD")).Checked = true;
                }
                else
                {
                    ((CheckBox)itm.FindControl("COD")).Checked = false;
                ...
                if (((CheckBox)itm.FindControl("COD")).Checked)
                {
                    if (txtcodidlist.Text.ToString().Contains(((HiddenField)itm.FindControl("HFcodordernum")).Value.ToString()) == false)
                    {
                        if (orderlist != "")
                        {
                            orderlist = orderlist + "," + ((HiddenField)itm.FindControl("HFcodordernum")).Value.ToString();
                ...
                {
                    if (txtcodidlist.Text.ToString().Contains(((HiddenField)itm.FindControl("HFcodordernum")).Value.ToString()) == true)
                    {
                        txtcodidlist.Text = txtcodidlist.Text.ToString().Replace(((HiddenField)itm.FindControl("HFcodordernum")).Value.ToString(), "");
                    }
                    txtcodidlist.Text = txtcodidlist.Text.ToString().Replace(",,", ",");
                    if (txtcodidlist.Text.ToString().EndsWith(","))
                    {
                        txtcodidlist.Text = txtcodidlist.Text.ToString().Remove(txtcodidlist.Text.ToString().Length - 1);
                    }
                    if (txtcodidlist.Text.ToString().StartsWith(","))
                    {
                        txtcodidlist.Text = txtcodidlist.Text.ToString().Remove(0, 1);
                    }
                }
            }
            if (orderlist != "")
            {
                if (txtcodidlist.Text != "")
                {
                    txtcodidlist.Text = txtcodidlist.Text + "," + orderlist;
                }
                else
                {
                    txtcodidlist.Text = orderlist;
                }
            }
            HFcodorderlist.Value = txtcodidlist.Text.ToString();
        }
        protected void btncodsubmit_OnClick(object sender, EventArgs e)
        {
            //ServiceCODOrderupdate();
        }
    }
